...
int main(int argc, char** argv)
{
	...
	*h_NormW = sqrt(*h_NormW);                                        // square root
	...
	// compute lambda
	ComputeLambda<<<blocksPerGrid, threadsPerBlock, sharedMemSize>>>(d_VecV, d_VecW, d_lambda, N);
	cudaDeviceSynchronize();
	...
	// break condition
	if (abs(*h_lambda-Oldlambda) < EPS) break;
	Oldlambda = *h_lambda;
    }
    ...
    runtime = (t_end.tv_sec - t_start.tv_sec) + 1e-9*(t_end.tv_nsec - t_start.tv_nsec);
    runtime_exm = (t_end.tv_sec - t_start.tv_sec) + 1e-9*(t_end.tv_nsec - t_start.tv_nsec)
	     -( t_end_c0.tv_sec - t_start_c0.tv_sec + 1e-9*(t_end_c0.tv_nsec - t_start_c0.tv_nsec) ) 
